...
En tenant compte de l'historique ci-dessus si pertinent, décris cette sortie en une seule phrase très simple et naturelle en français. Commence par "Dans ce dossier, il y a" ou similaire."""
    ...
    elif prompt_type == "security_analysis": # Prompt affiné pour la sécurité
...
# --- NOUVELLE Fonction pour suggérer des outils ---
def get_tool_suggestions(task_description: str, model_name: str):
    """
    Demande à Ollama de suggérer des outils pour une tâche de sécurité donnée.
    """
    # Prompt spécifique pour cette tâche
    # Note: Pas besoin de l'historique de conversation ici, sauf si la tâche le nécessite explicitement.
...
Formate la sortie clairement, par exemple sous forme de liste numérotée. Concentre-toi uniquement sur les outils CLI pertinents. Sois concis et précis."""
    ...
    payload = {
        "model": model_name,
        "system": SYSTEM_PROMPT, # On garde le rôle système général
        "prompt": prompt,
        "stream": False # Pas besoin de streamer pour une liste d'outils
    }

    # Appel API standard non-streamé
    try:
        # Le message d'attente est géré par main.py
        response = requests.post(OLLAMA_API_URL, json=payload, timeout=OLLAMA_TIMEOUT_TRANSLATE) # Utilise timeout traduction
        response.raise_for_status()
        data = response.json()
        suggestions_text = data.get("response", "").strip()

        if not suggestions_text:
            return "(L'IA n'a retourné aucune suggestion.)"

        return suggestions_text

    # --- Gestion des erreurs (simplifiée) ---
    except requests.exceptions.ConnectionError:
        return (f"{COLOR_RED}{COLOR_BOLD}Erreur : Connexion Ollama impossible.{COLOR_RESET}\n"
                f"{COLOR_RED}Vérifiez lancement et URL {OLLAMA_API_URL}{COLOR_RESET}")
    except requests.exceptions.Timeout:
        return f"{COLOR_RED}{COLOR_BOLD}Erreur : Timeout ({OLLAMA_TIMEOUT_TRANSLATE}s) Ollama (Suggestion).{COLOR_RESET}"
    except requests.exceptions.RequestException as e:
        return f"{COLOR_RED}{COLOR_BOLD}Erreur requête Ollama (Suggestion): {e}{COLOR_RESET}"
    except Exception as e:
        return f"{COLOR_RED}{COLOR_BOLD}Erreur inattendue (Suggestion Ollama): {type(e).__name__}: {e}{COLOR_RESET}"
